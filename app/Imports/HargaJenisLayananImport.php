...
class HargaJenisLayananImport implements ToModel, WithHeadingRow
{
    ...
    */
    public function model(array $row)
    {
        ...
        $namaPakaian = JenisCucian::withTrashed()->where('nama', $row['nama_pakaian'])->where('cabang_id', $cabang->id)->first();
        $hargaJenisLayanan = HargaJenisLayanan::withTrashed()->where('jenis_layanan_id', $namaLayanan->id)->where('jenis_cucian_id', $namaPakaian->id)->where('cabang_id', $cabang->id)->first();
        ...
        if (empty($hargaJenisLayanan) && ($row['jenis_satuan'] == 'Kg' || $row['jenis_satuan'] == 'Perjalanan')) {
            return new HargaJenisLayanan([
                ...
                'jenis_cucian_id' => $namaPakaian->id,
                'cabang_id' => $cabang->id,
            ]);
        }
    }
