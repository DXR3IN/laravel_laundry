...
class HargaJenisLayananImport implements ToModel, WithHeadingRow
{
    ...
    public function model(array $row)
    {
        ...
        $namaCucian = JenisCucian::withTrashed()->where('nama', $row['nama_cucian'])->where('cabang_id', $cabang->id)->first();
        $hargaJenisLayanan = HargaJenisLayanan::withTrashed()->where('jenis_layanan_id', $namaLayanan->id)->where('jenis_cucian_id', $namaCucian->id)->where('cabang_id', $cabang->id)->first();
